...
import { getOrCreateUser, updateUserPreferences, getPlaces, getCategories, getQuickPrompts, getReviews, getUserNotifications, createNotification, markNotificationRead, getTrips, createTrip, getChatSessions, createChatSession, addChatMessage, type UserRow, type NotificationRow } from './db.js';
import { generatePersonalizedPlaces, generatePersonalizedPrompts, generatePersonalizedWelcome, generatePersonalizedNotifications } from './ai-generator.js';
...
function getDeviceId(req: express.Request): string {
  ...
  return req.headers['device-id'] as string || req.ip || req.socket.remoteAddress || 'unknown';
}
...
app.post('/api/user/preferences', async (req, res) => {
  try {
    ...
    if (personalizedData.notifications && personalizedData.notifications.length > 0) {
...
app.get('/api/personalized', async (req, res) => {
  try {
    ...
    if (user.has_personalized !== 1) {
      // Return default data for users who skipped /welcome
    ...
    // Try to generate personalized data
    const personalizedData = await generatePersonalizedData({ preferences, travelStyles, budget });
...
async function generatePersonalizedData(userData: {
  ...
  try {
    // Generate personalized places
    const aiPlaces = await generatePersonalizedPlaces(userData);

    // Generate personalized prompts
    const aiPrompts = await generatePersonalizedPrompts(userData);

    // Generate personalized welcome message
    const aiWelcome = await generatePersonalizedWelcome({
      name: 'Bạn',
      ...userData,
    });

    // Generate personalized notifications
    const aiNotifications = await generatePersonalizedNotifications(userData);
    ...
    return {
      ...
      notifications: aiNotifications.length > 0 ? aiNotifications : defaultData.notifications,
    };
  } catch (error) {
...
app.get('/api/notifications', (req, res) => {
  try {
    ...
    res.json(notifications.map((n: NotificationRow) => ({
      id: n.id,
      type: n.type,
      title: n.title,
      content: n.content,
...
app.post('/api/chat', rateLimiter, async (req, res) => {
  try {
    const { message, history = [], imageBase64, sessionId } = req.body;
    ...
    const messages: OpenAI.ChatCompletionMessageParam[] = [
      ...
      ...historyMessages,
      { role: 'user', content: userContent.length === 1 && userContent[0].type === 'text' ? (userContent[0] as OpenAI.ChatCompletionContentPartText).text : userContent },
    ];
    ...
    // Save assistant message to session if sessionId provided
    if (sessionId) {
