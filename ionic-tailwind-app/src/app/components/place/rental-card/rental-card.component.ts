...
@Component({
  ...
  template: `
    <div
      ...
      class="block cursor-pointer focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-1 rounded-2xl"
    >
      <div
      ...
      >
        <div class="relative h-36 overflow-hidden">
          ...
          <div class="absolute bottom-2 left-3">
            <span
              class="text-[11px] font-semibold text-white bg-black/80 backdrop-blur-sm px-2.5 py-1 rounded-full"
            >
              🛵 {{ place.pricePerDay }}
            </span>
          </div>
          <div
            *ngIf="place.rating"
            class="absolute top-3 right-3 flex items-center gap-1 bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded-full"
          >
            <svg
        ...
        <div class="p-3">
          ...
          <div
            *ngIf="place.vehicleTypes?.length"
            class="flex flex-wrap gap-1 mb-3"
          >
            <span
              *ngFor="let type of vehicleBadges()"
              class="text-[10px] px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full"
            >
              {{ type }}
            </span>
            <span
              *ngIf="(place.vehicleTypes?.length || 0) > 2"
          ...
          <div class="flex gap-2 mt-4" (click)="$event.stopPropagation()">
            ...
            <button
              ...
              aria-label="Xem chỉ đường trên bản đồ"
