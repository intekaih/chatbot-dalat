...
import { Observable, Subject, BehaviorSubject, map, catchError, throwError, of } from 'rxjs';
...
export class AIService {
   ...
   */
  chat(messages: ChatMessage[], model?: string, imageBase64?: string): Observable<ChatCompletionResponse> {
    ...
    return this.http.post<{ reply: string; suggestedPlace: unknown }>(
      `${AI_CONFIG.baseUrl}/api/chat`,
      body,
  ...
  private messagesToBeFormat(messages: ChatMessage[]): { message: string; history: { role: string; content: string }[] } {
    const withoutSystem = messages.filter(m => m.role !== 'system');
    if (withoutSystem.length === 0) {
      return { message: '', history: [] };
    }
    const last = withoutSystem[withoutSystem.length - 1];
    const history = withoutSystem.slice(0, -1).map(m => ({ role: m.role, content: m.content }));
    return {
  ...
  extractPlace(message: string, reply: string): Observable<{ name?: string; address?: string; description?: string } | null> {
    return this.http.post<{ suggestedPlace: { name?: string; address?: string; description?: string } | null }>(
      `${AI_CONFIG.baseUrl}/api/extract-place`,
      { message, reply },
      { headers: this.getHeaders() }
    ).pipe(
