...
@Component({
  ...
  template: `
    <div class="bg-white">
      <!-- Header -->
      <div class="px-4 pt-12 pb-4 border-b border-gray-100 flex items-center justify-between">
      ...
      <!-- Places Tab -->
      <div *ngIf="activeTab === 'places'" class="p-4">
        <div class="space-y-4">
          <app-place-card 
      ...
      <!-- Trips Tab -->
      <div *ngIf="activeTab === 'trips'" class="p-4">
        <div class="space-y-4">
          <div 
            *ngFor="let trip of trips"
            class="flex items-center gap-4 p-3 border border-gray-100 rounded-xl cursor-pointer"
            (click)="goToTrip(trip.id)"
          >
            <img [src]="trip.coverImage" class="w-20 h-20 rounded-lg object-cover" />
            <div class="flex-1 min-w-0">
              <h3 class="font-medium text-gray-900 mb-1">{{ trip.title }}</h3>
              <p class="text-xs text-gray-500 mb-1">{{ trip.startDate }} - {{ trip.endDate }}</p>
              <span 
                class="inline-block px-2 py-0.5 rounded-full text-xs"
                [class]="trip.status === 'upcoming' ? 'bg-blue-100 text-blue-700' : trip.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'"
              >
        ...
        <a 
          *ngIf="trips.length > 0"
          href="/trips"
          class="flex items-center justify-between mt-6 p-4 border border-gray-100 rounded-xl hover:bg-gray-50"
        >
...
export class FavoritesPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apiService.getPlaces().subscribe({
      next: (places) => {
        this.favoritePlaces = places.slice(0, 3);
      }
    });
  ...
  createNewTrip(event: Event) {
    ...
    this.router.navigate(['/home/chat'], { state: { prompt: 'Tạo lịch trình mới' } });
  }
