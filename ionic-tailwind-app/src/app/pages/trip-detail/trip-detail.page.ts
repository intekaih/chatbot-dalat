...
import { Trip, TripDay, TripItineraryItem } from '../../services/api.service';
    ...
    <div class="bg-white">
      ...
      <div class="relative aspect-video overflow-hidden">
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4">
          <h1 class="text-2xl font-semibold text-white mb-2">{{ trip?.title }}</h1>
          <p class="text-white/80 text-sm mb-3">{{ trip?.destination }}</p>
          <span 
            class="px-2 py-0.5 rounded-full text-xs text-white"
            [class]="trip?.status === 'upcoming' ? 'bg-blue-500/80' : trip?.status === 'completed' ? 'bg-green-500/80' : 'bg-amber-500/80'"
          >
            {{ trip?.status === 'upcoming' ? 'Sắp tới' : trip?.status === 'completed' ? 'Đã đi' : 'Đang đi' }}
          </span>
        </div>
      </div>
      ...
      <div class="px-4 py-4 border-b border-gray-100">
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-gray-600">{{ trip?.startDate }} - {{ trip?.endDate }}</span>
          </div>
          <span class="px-2 py-0.5 bg-gray-100 rounded-full text-xs">{{ trip?.days?.length }}N{{ trip?.days?.length! - 1 }}Đ</span>
        </div>
      </div>

      <!-- Notes Banner -->
      <div *ngIf="trip?.notes" class="px-4 py-3 bg-amber-50 border-b border-amber-100">
      ...
      <div class="px-4 py-4 border-b border-gray-100">
        <div class="flex p-1 bg-gray-100 rounded-2xl">
          <button 
            ...
            [class]="activeTab === 'itinerary' ? 'bg-white shadow-sm' : 'text-gray-500'"
          >
            🗓️ Lịch trình
          </button>
          <button 
            ...
            class="flex-1 py-2 rounded-xl text-sm font-medium transition-colors"
            [class]="activeTab === 'budget' ? 'bg-white shadow-sm' : 'text-gray-500'"
          >
            💰 Ngân sách
          </button>
        </div>
      ...
      <div *ngIf="activeTab === 'itinerary'" class="p-4">
        <div *ngFor="let day of trip?.days; let i = index" class="mb-4">
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <span class="text-sm text-gray-500">{{ day.totalCost | number:'1.0-0' }}đ</span>
              <svg 
          ...
          <div *ngIf="expandedDays[i]" class="space-y-3 pl-4">
            ...
            >
              ...
              <div class="flex flex-col items-center">
                ...
                <div *ngIf="j < day.items.length - 1" class="w-px h-6 bg-gray-200"></div>
              </div>
              ...
              <div class="flex-1 pb-3">
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span class="text-xs text-gray-400">{{ getItemTypeLabel(item.type) }}</span>
                </div>
                <h4 class="text-sm font-medium text-gray-900">{{ item.title }}</h4>
                <p *ngIf="item.description" class="text-xs text-gray-500">{{ item.description }}</p>
                <p *ngIf="item.cost && item.cost > 0" class="text-xs text-gray-600 mt-1">{{ item.cost | number:'1.0-0' }}đ</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div *ngIf="activeTab === 'budget'" class="p-4">
        ...
        <div class="bg-gray-50 rounded-xl p-4 mb-4">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="font-semibold">{{ trip?.spent | number:'1.0-0' }}đ</span>
          </div>
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm text-gray-600">Ngân sách</span>
            <span class="text-sm">{{ trip?.totalBudget | number:'1.0-0' }}đ</span>
          </div>
          ...
          <div class="h-2 bg-gray-200 rounded-full mb-2 overflow-hidden">
            <div 
              ...
              [class]="getBudgetBarColor(trip?.spent || 0, trip?.totalBudget || 1)"
              [style.width.%]="((trip?.spent || 0) / (trip?.totalBudget || 1)) * 100"
            ></div>
          </div>
          
          <div class="flex items-center justify-between text-sm">
            <span class="text-gray-500">{{ ((trip?.spent || 0) / (trip?.totalBudget || 1) * 100) | number:'1.0-0' }}% đã sử dụng</span>
            <span [class]="(trip?.spent || 0) > (trip?.totalBudget || 0) ? 'text-red-600' : 'text-green-600'">
              {{ (trip?.totalBudget || 0) - (trip?.spent || 0) | number:'1.0-0' }}đ còn lại
            </span>
          </div>
        </div>
        ...
        <!-- Category Breakdown -->
        <div class="space-y-3">
          <div *ngFor="let cat of trip?.budgetCategories" class="flex items-center gap-3">
            ...
            <div class="flex-1">
              <div class="flex items-center justify-between mb-1">
                ...
                <span class="text-xs text-gray-500">{{ cat.spent | number:'1.0-0' }}đ / {{ cat.budget | number:'1.0-0' }}đ</span>
              </div>
...
export class TripDetailPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.trip = MOCK_TRIPS.find(t => t.id === id);
